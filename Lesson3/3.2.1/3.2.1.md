...
```
## Step 7 @unplugged
Return to the Code Builder window and on line 4 delete the word Right from the agent.turn(TurnDirection.Right).
...
## Step 14 
Change the coordinates to be ~1 ~20 ~-1 by changing the 3 number parameters in the positions.create to 1, 20, -1.
#### ~ tutorialhint
```javascript
player.onChat("sayHello", function () {
    player.say("Hello player!")
