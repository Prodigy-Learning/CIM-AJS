...
## Step 3 
From the ``||PLAYER: PLAYER||`` drawer drag out ``||PLAYER: run code on chat command jump num1 num2 num3 ||`` block and drop it onto the code editor surface
#### ~ tutorialhint
```javascript
...
We want to turn him 180 degrees so he can move back down the line of blocks so add another ``||AGENT: agent move direction by 1||`` block and ``||AGENT: agent turn(TurnDirection.Left)||`` block after the existing
one.
#### ~ tutorialhint
```javascript
player.onChat("farm", function () {
...
The second paramter is the direction the Agent is to face - this is WEST by default - change this to EASE to orientate the Agent correctly.
