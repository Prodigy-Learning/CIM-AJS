...
```
## Step 7
From the Agent drawer place a destroy(FORWARD) block and a move(FORWARD, 1) block inside the while loop on lines 4 and 5.
#### ~ tutorialhint
```javascript
player.onChat("mine", function () {
    agent.teleportToPlayer()
...
```
##Step 13
After the == enter the text GOLD_BLOCK to test for a Gold Block.
...
```
## Step 16
As we done earlier after the == enter the text AIR to test for an air Block.
