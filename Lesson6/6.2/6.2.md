...
The 
```javascript 
while (agent.detect(AgentDetection.Block, SixDirection.Forward) == false) { ... } 
```
while statement will loop around while the agent does not detect a block in front of it.
